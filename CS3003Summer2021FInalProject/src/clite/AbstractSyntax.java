...
class Program {
    ...
    String inner_display(String spc, String spcing, Object node) {
        ...
        if (node instanceof Assignment) {
            ...
            // Target should have been declared. Check globals (class-level variable) and
            // check declarations in this function.
            Boolean found = false;
            for (Declaration d : globals) {
                if (myTarget.equals(d.v.id)) {
                    found = true;
                    break;
                }
            }
            // ToDo: The problem is that we don't know what function we're in. We can't
            // check local declarations (current function and current block(s)!) and we
            // can't check parameter declarations
            // So we fixed that by adding a class-level variable that tracks the current
            // function we are in.
            if (currentFunction != null) { // This should not be necessary because an assignment will always be in a
                                           // function.
            ...
            if (!found) {
                System.err.println("*** Undeclared variable: " + myTarget);
            }
...
class Block extends Statement {
    ...
    /*
     * public void display() { for (Statement s : members) { s.display(); } }
     */
}
...
class Conditional extends Statement {
    ...
    /*
     * public void display() { System.out.println("Conditional: ");
     * System.out.print("\t"); test.display(); System.out.println("\t");
     * thenbranch.display(); System.out.println("\t"); elsebranch.display(); }
     */
}
...
class ArrayRef extends VariableRef {
    ...
    // I think the reason the ArrayRefs are not being matched. Is because I have not
    // implemented the function hashCode, for ArrayRef
}
...
class Operator {
    ...
    final static String DOUBLE_MOD = "DOUBLE%";
    // UnaryOp = !
    ...
    boolean ArithmeticOp() {
        return val.equals(PLUS) || val.equals(MINUS) || val.equals(TIMES) || val.equals(DIV) || val.equals(INT_PLUS)
                || val.equals(INT_MINUS) || val.equals(INT_TIMES) || val.equals(INT_DIV) || val.equals(INT_MOD)
                || val.equals(FLOAT_PLUS) || val.equals(FLOAT_MINUS) || val.equals(FLOAT_TIMES) || val.equals(FLOAT_DIV)
                || val.equals(FLOAT_MOD) || val.equals(DOUBLE_PLUS) || val.equals(DOUBLE_MINUS)
                || val.equals(DOUBLE_TIMES) || val.equals(DOUBLE_DIV) || val.equals(DOUBLE_MOD);
    }
    ...
    boolean NegateOp() {
        return (val.equals(NEG) || val.equals(INT_NEG) || val.equals(FLOAT_NEG) || val.equals(DOUBLE_NEG));
    }
    ...
    final static String intMap[][] = { { PLUS, INT_PLUS }, { MINUS, INT_MINUS }, { TIMES, INT_TIMES }, { DIV, INT_DIV },
            { MOD, INT_MOD }, { EQ, INT_EQ }, { NE, INT_NE }, { LT, INT_LT }, { LE, INT_LE }, { GT, INT_GT },
            { GE, INT_GE }, { NEG, INT_NEG }, { FLOAT, I2F }, { CHAR, I2C } };

    final static String floatMap[][] = { { PLUS, FLOAT_PLUS }, { MINUS, FLOAT_MINUS }, { TIMES, FLOAT_TIMES },
            { DIV, FLOAT_DIV }, { MOD, INT_MOD }, { EQ, FLOAT_EQ }, { NE, FLOAT_NE }, { LT, FLOAT_LT },
            { LE, FLOAT_LE }, { GT, FLOAT_GT }, { GE, FLOAT_GE }, { NEG, FLOAT_NEG }, { INT, F2I } };
    ...
    final static String boolMap[][] = { { EQ, BOOL_EQ }, { NE, BOOL_NE }, { LT, BOOL_LT }, { LE, BOOL_LE },
            { GT, BOOL_GT }, { GE, BOOL_GE }, { OR, OR }, { AND, AND }, { NOT, NOT } };

    final static String doubleMap[][] = { { PLUS, DOUBLE_PLUS }, { MINUS, DOUBLE_MINUS }, { TIMES, DOUBLE_TIMES },
            { DIV, DOUBLE_DIV }, { MOD, DOUBLE_MOD }, { EQ, DOUBLE_EQ }, { NE, DOUBLE_NE }, { LT, DOUBLE_LT },
            { LE, DOUBLE_LE }, { GT, DOUBLE_GT }, { GE, DOUBLE_GE }, { NEG, DOUBLE_NEG }, { INT, D2I } };
