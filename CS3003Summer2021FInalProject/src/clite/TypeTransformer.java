...
public class TypeTransformer {
	...
	public static Expression T(Expression e, TypeMap tm) {
		...
		if (e instanceof Unary) {
			...
			if (typ == Type.INT)
				return new Unary(Operator.intMap(u.op.val), t);
			else if (typ == Type.FLOAT)
				return new Unary(Operator.floatMap(u.op.val), t);
			else if (typ == Type.CHAR)
				return new Unary(Operator.charMap(u.op.val), t);
			else if (typ == Type.DOUBLE)
				return new Unary(Operator.doubleMap(u.op.val), t);
			else if (typ == Type.BOOL)
				return new Unary(Operator.boolMap(u.op.val), t);
			throw new IllegalArgumentException("should never reach here");
		}
