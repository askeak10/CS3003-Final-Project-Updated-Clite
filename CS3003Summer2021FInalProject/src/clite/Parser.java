...
public class Parser {
	...
	private void error(TokenType tok) {
		System.err.println("Syntax error: expecting: " + tok + "; saw: " + token + " at " + lexer.get_lineno() + ":"
				+ lexer.get_col());
		System.exit(1);
	}
	...
	public Program program() {
		...
		// Now we have the global declarations, if there were any. There should be a
		// main function here.
	...
	private void parameter(Declarations dec) {
		...
		if (token.type().equals(TokenType.RightParen)) {
			...
			Type t = type(); // this is throwing an error. and accidentally requires all functions to have
								// formal parameters
			token = lexer.next();
			String id = match(TokenType.Identifier);
	...
	private Type type() {
		...
		if (token.type().equals(TokenType.Int)) {
			...
			error("int | bool | float | char | double");
		// student exercise
		return t;
	}
	...
	private Statement statement() {
		...
		// CallStatement | ReturnStatement | Print
	...
	private Expression addition() {
		...
		while (isAddOp()) {
			Operator op = new Operator(match(token.type()));
			Expression mod2 = term();
			e = new Binary(op, e, mod2);
		}
		return e;
	}
	...
	private Expression primary() {
		...
		if (token.type().equals(TokenType.Identifier)) {
			...
			if (token.type().equals(TokenType.LeftBracket)) {
				...
				e = callExpression(id); // Because call has to be both a statement and an expression we distinguish its
										// superclass with two methods.
			} else {
				e = new Variable(id);
			}
		} else if (isLiteral()) {
	...
	private boolean isModOp() {
		return token.type().equals(TokenType.Modulo);
	}
	...
	private boolean isType() {
		return token.type().equals(TokenType.Int) || token.type().equals(TokenType.Bool)
				...
				|| token.type().equals(TokenType.Void) || token.type().equals(TokenType.Double);
	}
	...
	public static void main(String args[]) {
		// Parser parser = new Parser(new Lexer(args[0])); //Picks the file name and
		// feeds it to the lexer.
		// Parser parser = new Parser(new Lexer("hello.cpp"));
		Parser parser = new Parser(new Lexer(
				"C:\\Users\\askeak\\OneDrive - University of Cincinnati\\ProgLang\\CS3003Summer2021FinalProject\\CS3003Summer2021FInalProject\\test.cpp"));
