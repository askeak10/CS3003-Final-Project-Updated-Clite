...
public class JasminFile extends FileWriter {
	...
	public void write_globals(Declarations globals) throws IOException {
		for (Declaration global : globals) {
			...
			if (global.t.equals(Type.INT) || global.t.equals(Type.BOOL) || global.t.equals(Type.CHAR))
				global_sig += " I";
			else if (global.t.equals(Type.DOUBLE))
				global_sig += "D";
			else
				global_sig += " V";
			writeln(global_sig);
	...
	public void main_preamble(Declarations locals) throws IOException {
		...
		writeln("\t.limit stack" + " " + "14"
				+ " ;(Hack!) Although it is possible to statically determine the size of the stack based off counting expression information \n\t\t\t;in a function it's too much work for now. ");
	 ...
	 * limit should be the number of parameters plus the numbers of local variables
	 * declared (How will globals work?)
	 */
	public void function_preamble(String func_name, Type return_type, Declarations params, Declarations locals)
			throws IOException {
		...
		method_sig += ")";
		if (return_type.equals(Type.INT) || return_type.equals(Type.CHAR) || return_type.equals(Type.BOOL))
			method_sig += "I";
